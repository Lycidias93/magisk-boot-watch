...
grep -q '^version=0.2.4$' "$tmp/module.prop"
grep -q '^versionCode=24$' "$tmp/module.prop"
grep -q '^updateJson=https://raw.githubusercontent.com/Lycidias93/magisk-boot-watch/main/update.json$' "$tmp/module.prop"
grep -q 'VERSION="0.2.4"' "$tmp/boot-watch.sh"
grep -q 'VERSION_CODE="24"' "$tmp/boot-watch.sh"
...
for f in service.sh boot-watch.sh result-log-export.sh action.sh manual-collect.sh customize.sh uninstall.sh; do
  test -x "$tmp/$f"
done
echo "RESULT: MAGISK_BOOT_WATCH_VERIFY_PASS version=0.2.4"
